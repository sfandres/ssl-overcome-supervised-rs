...
def tsne_computation_and_plot(embeddings, labels, seed, plot='all', filename='', save_2d=False):
    ...
    # Create dataframe with the resulting data.
    df = pd.DataFrame()
    df['tsne_2d_x'] = tsne_results[:, 0]
    df['tsne_2d_y'] = tsne_results[:, 1]
    df['labels'] = labels
    ...
    # Create dataframe with the resulting data.
    df = pd.DataFrame()
    df['tsne_3d_x'] = tsne_results[:, 0]
    df['tsne_3d_y'] = tsne_results[:, 1]
    df['tsne_3d_z'] = tsne_results[:, 2]
    df['labels'] = labels
    ...
    if plot == '3d' or plot == "23d" or plot == 'all':
        ...
        ax.set_xlabel('tsne_3d_x')
        ax.set_ylabel('tsne_3d_y')
        ax.set_zlabel('tsne_3d_z')
        plt.show()

    # 3-D plot with pyplot.
    if plot == '3d-plotly' or plot == 'all':
        fig = px.scatter_3d(df, x='tsne_3d_x',
                            y='tsne_3d_y', z='tsne_3d_z',
                            color='labels',
                            width=1000, height=800)  # symbol='labels'
