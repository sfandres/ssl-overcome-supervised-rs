...
def main(args):
    ...
    filename = os.path.join(args.output, f'result_table_{task}_v2.csv')
    ...
    with open(filename, 'a', newline='') as csvfile:
        ...
        if task == 'multiclass':
            csvwriter.writerow(['TR (\%)', 'Model'] + ['F1 per class' for x in range(10)] + ['Micro F1', 'Macro F1', 'Weighted F1'])
            csvwriter.writerow(['TR (\%)', 'Model'] + [x for x in range(10)] + ['Micro F1', 'Macro F1', 'Weighted F1'])
        elif task == 'multilabel':
            csvwriter.writerow(['TR (\%)', 'Model'] + ['RMSE per class' for x in range(10)] + ['RMSE'])
            csvwriter.writerow(['TR (\%)', 'Model'] + [x for x in range(10)] + ['RMSE'])
    ...
    for transfer in transfer_learning_algs:
        ...
        for nf, ratio in enumerate(filtered_dirs):
            ...
            for model in models:
                ...
                # Final row.
                if task == 'multiclass':
                    row = [ratio[:-1], model_name] + per_class_res + [f1_micro_res] + [f1_macro_res] + [f1_weighted_res]
                elif task == 'multilabel':
                    row = [ratio[:-1], model_name] + per_class_res + [rmse_res]
